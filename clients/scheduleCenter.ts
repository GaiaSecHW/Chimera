...
import { ScheduleCenterUserTaskStats } from '../types/types';
...
export const scheduleCenterApi = {
  ...
  getUserTaskStats: async (projectId: string): Promise<ScheduleCenterUserTaskStats> =>
    handleResponse(await fetch(
      `${API_BASE}/api/chirmera-platform-schedule/projects/${encodeURIComponent(projectId)}/user-tasks/stats`,
      { headers: getHeaders() },
    )),
  ...
  changeUserTaskProject: async (projectId: string, taskId: string, newProjectId: string): Promise<any> =>
    handleResponse(await fetch(`${API_BASE}/api/chirmera-platform-schedule/projects/${encodeURIComponent(projectId)}/user-tasks/${encodeURIComponent(taskId)}/project`, {
      method: 'PATCH',
      headers: getHeaders(),
