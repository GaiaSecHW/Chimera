...
export const TemplateLlmBindingEditor: React.FC<TemplateLlmBindingEditorProps> = ({
  ...
  useEffect(() => {
    ...
    const loadProviders = async () => {
      ...
      try {
        const data = await environmentApi.environment.listTemplateLlmProviders(projectId || '');
        if (!cancelled) {
  ...
  useEffect(() => {
    ...
    const loadPreview = async () => {
      ...
      setPreviewError('');
      try {
        const data = await environmentApi.environment.previewTemplateLlmBinding(projectId || '', selectedProviderKeys, targetServices);
