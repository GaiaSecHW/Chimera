...
export function FeedbackReplyDialog({ feedback, open, onClose, onReplied }: FeedbackReplyDialogProps) {
  ...
  const handleSubmit = async () => {
    if (!stripHtml(replyContent)) {
      ui.notify('请输入答复内容', 'warning');
      return;
    }
    setSubmitting(true);
    try {
      await feedbackApi.reply(feedback.id, replyContent);
      ui.notify('答复已提交', 'success');
      onReplied();
      onClose();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50" onClick={onClose}>
      <div
        className="w-full max-w-2xl max-h-[80vh] flex flex-col rounded-2xl border border-theme-border bg-theme-surface shadow-panel"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between p-4 border-b border-theme-border">
          <h3 className="text-lg font-semibold text-theme-text">处理反馈</h3>
