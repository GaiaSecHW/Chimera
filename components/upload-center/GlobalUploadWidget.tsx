  ...
  return (
    <div className="fixed bottom-4 right-4 z-[80] w-[360px] max-w-[calc(100vw-1rem)]">
      <div className="rounded-2xl border border-theme-border bg-theme-surface shadow-panel overflow-hidden">
        <button
          type="button"
          className="w-full px-4 py-3 flex items-start justify-between gap-3 text-left hover:bg-theme-elevated transition-colors"
          onClick={() => setExpanded((prev) => !prev)}
        >
          <div className="flex items-start gap-3 min-w-0">
            <div className="w-9 h-9 rounded-xl bg-logo-surface text-theme-text-inverse flex items-center justify-center shrink-0 shadow-brand">
              <UploadCloud size={16} />
            </div>
            <div className="min-w-0">
              <div className="text-xs font-bold text-theme-text-primary">{summaryText}</div>
          ...
          <div className="shrink-0 mt-1 text-theme-text-muted">
            {expanded ? <ChevronDown size={16} /> : <ChevronUp size={16} />}
          </div>
        </button>

        <div className="h-1 bg-theme-elevated">
          <div className="h-full bg-brand-primary transition-all duration-300" style={{ width: `${totalProgress}%` }} />
        </div>

        {expanded && (
          <div className="border-t border-theme-border">
            <div className="px-4 py-2.5 flex items-center justify-between bg-theme-elevated">
            ...
            <div className="max-h-[360px] overflow-y-auto p-2 space-y-2">
              {snapshot.tasks.length === 0 && (
                <div className="px-2 py-6 text-center text-xs text-theme-text-muted">暂无上传任务</div>
              )}
              {snapshot.tasks.map((task) => {
                const taskProgress = percent(task.uploadedBytes, task.totalBytes || task.size);
                return (
                  <div key={task.id} className="p-2.5 rounded-xl border border-theme-border bg-theme-surface">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <div className="text-xs font-semibold text-theme-text-primary truncate">{task.name}</div>
                        <div className="text-[11px] text-theme-text-muted mt-0.5 truncate">{task.source}</div>
                      </div>
                    ...
                    <div className="mt-2 h-1.5 bg-theme-elevated rounded-full overflow-hidden">
                      <div className="h-full bg-brand-primary transition-all duration-200" style={{ width: `${taskProgress}%` }} />
                    </div>
                    <div className="mt-1.5 flex items-center justify-between text-[11px] text-theme-text-muted">
