...
export const SecurityAssessmentPage: React.FC = () => {
    ...
    <div className="p-10 space-y-10 animate-in fade-in duration-500 pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={idx} className="bg-theme-bg-app p-6 rounded-3xl border border-theme-border relative group overflow-hidden">
            ...
            <h4 className="font-black text-theme-text-primary relative z-10">{item.title}</h4>
                  ...
                  {assessments.map(item => (
                    <tr key={item.id} className="hover:bg-theme-elevated transition-all cursor-pointer">
                      <td className="px-8 py-5">
                        <p className="text-sm font-black text-theme-text-secondary">{item.name}</p>
                        <p className="text-[10px] text-theme-text-muted font-bold mt-0.5">{item.date}</p>
                      </td>
                      <td className="px-6 py-5">
                        <span className="text-[10px] font-black text-theme-text-muted bg-theme-elevated px-2 py-1 rounded-lg uppercase">{item.type}</span>
                      </td>
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-black text-theme-text-muted">{item.progress}%</span>
                        </div>
                      </td>
 ...
 <div className="bg-theme-bg-app p-8 rounded-[2.5rem] border border-theme-border space-y-6 relative overflow-hidden">
            <Activity className="absolute right-[-20px] top-[-20px] w-40 h-40 text-slate-100 opacity-80 rotate-12" />
            <h4 className="text-[10px] font-black text-theme-text-muted uppercase tracking-widest">实时评估统计</h4>
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-theme-bg-app p-4 rounded-2xl border border-theme-border">
                <p className="text-[10px] font-bold text-theme-text-muted uppercase">累计评估</p>
                <p className="text-2xl font-black mt-1 text-theme-text-primary">128</p>
              </div>
              <div className="bg-theme-bg-app p-4 rounded-2xl border border-theme-border">
                <p className="text-[10px] font-bold text-theme-text-muted uppercase">平均得分</p>
                <p className="text-2xl font-black mt-1 text-green-400">82</p>
              </div>
            </div>
            <div className="pt-6 border-t border-theme-border">
              <div className="flex items-center justify-between mb-4">
                <span className="text-xs font-bold text-theme-text-muted">风险分布</span>
              </div>
 ...
 <div className="bg-theme-bg-app p-8 rounded-[2.5rem] border border-theme-border flex items-center gap-5 group cursor-pointer hover:border-blue-500 transition-all">
             <div className="w-14 h-14 bg-blue-500/15 text-blue-400 rounded-2xl flex items-center justify-center shrink-0 group-hover:bg-blue-600 group-hover:text-white transition-all">
               <Settings size={24} />
             </div>
             <div>
               <h4 className="font-black text-theme-text-primary">评估模版配置</h4>
