      ...
      <div className="mx-auto max-w-[1400px] space-y-4">
        ...
        <div className="overflow-hidden rounded-3xl border border-slate-200 bg-slate-800 shadow-panel">
          ...
          {loading ? (
            ...
            <div className="px-4 py-10 text-sm text-slate-400">暂无打包任务</div>
          ) : (
            sorted.map((task) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-xs text-slate-600">{task?.result?.mode || '--'}</div>
                  <div>
                    <button
                      type="button"
                      onClick={() => void handleDownload(task)}
                      disabled={!canDownload || busyTaskId === task.task_id}
                      className="inline-flex items-center gap-1 rounded-lg bg-slate-900 px-2.5 py-1.5 text-xs font-black text-white disabled:opacity-40"
                    >
                      <Download size={12} />
                      下载zip
                    </button>
