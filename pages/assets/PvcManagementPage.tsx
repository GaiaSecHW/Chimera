...
export const PvcManagementPage: React.FC<{ projectId: string }> = ({ projectId }) => {
  ...
  const loadPreviewState = async (
    ...
    if (mode === 'text') {
      const payload = await assetApi.resources.getPvcBrowserFile(resourceId, node.path);
      update({
        mode: 'text',
    ...
    if (mode === 'binary') {
      const payload = await assetApi.resources.getPvcBrowserFile(resourceId, node.path, 1024 * 1024);
    ...
    const blob = await assetApi.resources.fetchPvcBrowserPreviewBlob(resourceId, node.path);
  ...
  const refreshBrowser = async (resourceId: number, keepDirectoryPath?: string, keepPreviewPath?: string) => {
    ...
    const [detail, directory] = await Promise.all([
      assetApi.resources.getPvcResourceDetail(resourceId),
      assetApi.resources.getPvcBrowserChildren(resourceId, directoryPath),
    ]);
  ...
  const selectPvc = async (resourceId: number) => {
    ...
    const [detail, children] = await Promise.all([
      assetApi.resources.getPvcResourceDetail(resourceId),
      assetApi.resources.getPvcBrowserChildren(resourceId, '/'),
    ]);
    setSelectedPvcDetail(detail);
  ...
  const openDirectory = async (path: string) => {
    ...
    setBusy(`dir:${path}`);
    try {
      const children = await assetApi.resources.getPvcBrowserChildren(selectedPvcId, path);
      setCurrentDirectory({
        ...children,
        directories: sortNodes(children.directories),
        files: sortNodes(children.files),
  ...
  const handleCreatePvc = async (event: React.FormEvent) => {
    ...
    setCreateError('');
    try {
      await assetApi.resources.createManualPvc({ ...createForm, project_id: projectId });
  ...
  const handleDeletePvc = async (resource: OutputPvcDetail) => {
    ...
    try {
      await assetApi.resources.delete(resource.id);
      setSelectedPvcIds((prev) => {
        const next = new Set(prev);
  ...
  const handleDeleteSelectedPvcs = async () => {
    ...
    try {
      for (const item of selectedDeleteItems) {
        try {
          await assetApi.resources.delete(item.id);
        } catch (error: any) {
  ...
  const handleCreateDirectory = async (basePath?: string) => {
    ...
    try {
      ...
      await assetApi.resources.createPvcBrowserDirectory(pvc.id, targetPath, name);
  ...
  const submitArchiveUpload = async (event: React.FormEvent) => {
    ...
    try {
      for (const file of archiveFiles) {
        ...
        await assetApi.resources.upload(formData);
      }
  ...
  const handleUploadFiles = async (files: FileList | null) => {
    ...
    try {
      for (let i = 0; i < fileArray.length; i += 1) {
        ...
        try {
          await assetApi.resources.uploadPvcBrowserFile(
            pvc.id,
            uploadBasePath,
            file,
            {
  ...
  const handleRenameNode = async (node: PvcBrowserNode) => {
    ...
    try {
      await assetApi.resources.renamePvcBrowserNode(pvc.id, node.path, targetName);
  ...
  const handleMoveNode = async (node: PvcBrowserNode) => {
    ...
    try {
      await assetApi.resources.movePvcBrowserNode(pvc.id, node.path, normalized);
  ...
  const handleDeleteNode = async (node: PvcBrowserNode) => {
    ...
    try {
      await assetApi.resources.deletePvcBrowserNode(pvc.id, node.path);
      await refreshBrowser(pvc.id, currentDirectory?.current_path || '/');
      if (previewNode?.path === node.path) {
        setPreviewNode(null);
        setPreview({ mode: 'empty' });
  ...
  const handleDownloadNode = async (node: PvcBrowserNode) => {
    ...
    try {
      const blob = await assetApi.resources.fetchPvcBrowserDownloadBlob(pvc.id, node.path);
      const url = URL.createObjectURL(blob);
