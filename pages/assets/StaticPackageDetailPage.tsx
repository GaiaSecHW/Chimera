...
export const StaticPackageDetailPage: React.FC<StaticPackageDetailPageProps> = ({ packageId, onBack }) => {
  ...
  if (!data) return (
    <div className="p-20 text-center space-y-4">
      <div className="w-20 h-20 bg-theme-elevated rounded-3xl mx-auto flex items-center justify-center text-theme-text-faint">
        <Layers size={40} />
      </div>
      <h3 className="text-xl font-black text-theme-text-secondary">软件包未找到或已下线</h3>
      <button onClick={onBack} className="text-blue-400 font-black hover:underline px-6 py-2">返回资源中心</button>
    </div>
  );
 ...
 <div className="bg-theme-bg-app p-8 rounded-[2.5rem] border border-theme-border space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center py-2 border-b border-slate-50">
                <span className="text-xs font-bold text-theme-text-muted">操作系统</span>
                <span className="text-xs font-black text-theme-text-primary uppercase px-2 py-1 bg-theme-bg-app rounded-lg">{pkg.system}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-50">
                <span className="text-xs font-bold text-theme-text-muted">硬件架构</span>
                <span className="text-xs font-black text-blue-400 uppercase px-2 py-1 bg-blue-500/15 rounded-lg">{pkg.architecture}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-50">
                <span className="text-xs font-bold text-theme-text-muted">文件计数</span>
                <span className="text-xs font-black text-theme-text-primary">{pkg.file_count}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-50">
                <span className="text-xs font-bold text-theme-text-muted">物理大小</span>
                <span className="text-xs font-black text-theme-text-primary">{(pkg.total_size / 1024 / 1024).toFixed(2)} MB</span>
              </div>
              <div className="flex justify-between items-center py-2">
                <span className="text-xs font-bold text-theme-text-muted">下载热度</span>
                <span className="text-xs font-black text-theme-text-primary">{pkg.download_count} 次</span>
              </div>
            </div>
          </div>

          <div className="bg-theme-surface p-8 rounded-[2.5rem] text-theme-text-faint space-y-6">
            <h4 className="text-[10px] font-black text-theme-text-muted uppercase tracking-widest">底层安全信息</h4>
            <div className="space-y-4">
              <div className="space-y-1.5">
                <p className="text-[9px] font-black text-theme-text-muted uppercase">原始文件位置</p>
 <p className="text-[10px] font-mono break-all bg-slate-100/10 p-3 rounded-xl border border-slate-200/5 leading-relaxed text-theme-text-muted">
              ...
              <div className="space-y-1.5">
                <p className="text-[9px] font-black text-theme-text-muted uppercase">入库时间</p>
 ...
 <div className="bg-theme-bg-app rounded-[2.5rem] border border-theme-border overflow-hidden flex flex-col min-h-[600px]">
            <div className="p-8 border-b border-theme-border flex flex-col md:flex-row justify-between items-center gap-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-black text-theme-text-primary">包内组件索引</h3>
                  <p className="text-xs text-theme-text-muted font-bold uppercase tracking-tighter">TOTAL FILES: {data.total_files}</p>
                </div>
              </div>
              <div className="relative w-full md:w-80">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-3.5 bg-theme-bg-app rounded-2xl text-xs outline-none focus:ring-2 ring-blue-500/20 transition-all border-none font-medium"
                  value={fileSearch}
                      ...
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="min-w-0">
                            <p className="text-sm font-black text-theme-text-secondary truncate">{file.name}</p>
                            <p className="text-[10px] font-mono text-theme-text-muted truncate mt-0.5">{file.path}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-5">
                        <span className="text-[11px] font-black text-theme-text-muted bg-theme-bg-app px-2 py-1 rounded-lg">
                          {(file.size / 1024).toFixed(1)} KB
                        </span>
                      </td>
                      <td className="px-6 py-5">
                        <span className="text-[11px] font-black text-theme-text-muted">{file.download_count}</span>
                      </td>
                  ...
                  {filteredFiles.length === 0 && (
                    <tr>
                      <td colSpan={4} className="py-24 text-center">
                        <p className="text-theme-text-faint font-black uppercase tracking-widest text-xs">NO COMPONENTS MATCHED</p>
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            <div className="p-6 bg-theme-bg-app border-t border-theme-border flex justify-between items-center">
              ...
              <div className="flex gap-2">
                 <button className="px-3 py-1 bg-theme-bg-app border border-theme-border rounded-lg text-[10px] font-black text-theme-text-muted hover:text-blue-400 transition-all">PREV</button>
                 <button className="px-3 py-1 bg-theme-bg-app border border-theme-border rounded-lg text-[10px] font-black text-theme-text-muted hover:text-blue-400 transition-all">NEXT</button>
              </div>
            </div>
          </div>
        </div>
