...
export const EnvAiBatchSessionPage: React.FC<{ projectId: string }> = ({ projectId }) => {
  ...
  const loadBatches = async (silent = false) => {
    ...
    try {
      const result = await api.environment.listAiBatchSessions(projectId);
  ...
  const sendBatchMessage = async () => {
    ...
    try {
      if (transportMode === 'stream') {
        setStreamEvents([]);
        await api.environment.sendAiBatchMessageStream(activeBatchId, text, {
        ...
        await api.environment.sendAiBatchMessage(activeBatchId, text);
      }
      setMessage('');
