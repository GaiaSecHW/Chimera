...
const SectionCard: React.FC<{ title: string; subtitle?: string; actions?: React.ReactNode; children: React.ReactNode }> = ({ title, subtitle, actions, children }) => (
 <section className="rounded-2xl border border-theme-border bg-theme-elevated p-6 space-y-4">
...
const AgentInstanceList: React.FC<{
  ...
  return (
    <div className="space-y-2">
      ...
      <button onClick={add}
        className="inline-flex items-center gap-1.5 rounded-xl border border-dashed border-theme-border px-4 py-2 text-sm text-theme-text-muted hover:bg-theme-surface">
...
const PanelActions: React.FC<{ saving: boolean; onSave: () => void; onReset: () => void }> = ({ saving, onSave, onReset }) => (
  <div className="flex shrink-0 items-center gap-2">
    <button
      ...
      onClick={onReset}
      disabled={saving}
      className="rounded-xl border border-theme-border bg-theme-elevated px-3 py-2 text-xs font-semibold text-theme-text-secondary hover:bg-theme-surface disabled:opacity-50"
    >
      重置为默认
    </button>
    <button
...
export const CfgGuidedExploreConfigPage: React.FC<{ projectId: string; embedded?: boolean }> = ({ projectId, embedded = false }) => {
 ...
 <section className="rounded-[2rem] border border-theme-border bg-theme-elevated p-6">
          <h1 className="mt-3 text-3xl font-semibold tracking-tight text-theme-text-primary">分析配置</h1>
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <h2 className="text-xl font-semibold text-theme-text-primary">CFG Guided Explore参数配置</h2>
 ...
 className="inline-flex items-center gap-2 rounded-xl border border-theme-border bg-theme-elevated px-4 py-2.5 text-sm font-bold text-theme-text-primary hover:bg-theme-surface disabled:opacity-50"
            >
      ...
      {loading ? (
        <div className="inline-flex items-center gap-2 rounded-xl border border-theme-border bg-theme-elevated px-4 py-3 text-sm text-theme-text-secondary">
