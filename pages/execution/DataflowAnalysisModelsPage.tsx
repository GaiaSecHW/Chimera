...
import { DEFAULT_DFA_MODELS_CONFIG } from '../../clients/appDataflowAnalyse';
import { AppDfaModelEntry, AppDfaModelsConfig, AppDfaProviderConfig } from '../../types/types';
import { useUiFeedback } from '../../components/UiFeedback';

const API_TYPES = ['openai-completions', 'anthropic', 'google-gemini', 'azure-openai'];

const emptyProvider = (): AppDfaProviderConfig => ({
  baseUrl: '',
...
const emptyModel = (): AppDfaModelEntry => ({ id: '', reasoning: false });

const emptyConfig = (): AppDfaModelsConfig => ({ providers: {} });
...
const ProviderCard: React.FC<{
  name: string;
  config: AppDfaProviderConfig;
  onRename: (newName: string) => void;
  onChange: (config: AppDfaProviderConfig) => void;
  onRemove: () => void;
    ...
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      {expanded && (
        <div className="p-4 space-y-4">
          ...
          <ModelsList models={config.models} providerName={name}
...
export const DataflowAnalysisModelsPage: React.FC = () => {
  const { notify, feedbackNodes } = useUiFeedback();
  const dfaApi = api.domains.execution.appDataflowAnalyse;

  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [config, setConfig] = useState<AppDfaModelsConfig>(emptyConfig);
    ...
    dfaApi.getModels()
      ...
      .catch((err) => {
        if (!cancelled) {
          ...
          setConfig(DEFAULT_DFA_MODELS_CONFIG);
        }
      })
  ...
  const setProviders = (providers: Record<string, AppDfaProviderConfig>) =>
  ...
  const renameProvider = (oldName: string, newName: string) => {
    ...
    const next: Record<string, AppDfaProviderConfig> = {};
  ...
  const updateProvider = (name: string, updated: AppDfaProviderConfig) =>
    setProviders({ ...config.providers, [name]: updated });

  const handleSave = async () => {
    setSaving(true);
    try {
      const saved = await dfaApi.saveModels(config);
    ...
    <div className="px-8 pt-8 pb-10 space-y-6">
      ...
      <section className="rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-500">
          管理数据流分析引擎使用的 LLM 提供商及模型列表。引用格式：
