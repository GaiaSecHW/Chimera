...
const STAGE_STEP_TONES: Record<string, { card: string; dot: string; badge: string }> = {
  init: {
    card: 'border-cyan-500/20 bg-cyan-500/15',
    dot: 'border-cyan-500/20 bg-cyan-500/15 text-cyan-400',
    badge: 'border-cyan-500/20 bg-cyan-500/15 text-cyan-400',
  },
  worker: {
    card: 'border-indigo-500/20 bg-indigo-500/15',
    dot: 'border-indigo-500/20 bg-indigo-500/15 text-indigo-400',
    badge: 'border-indigo-500/20 bg-indigo-500/15 text-indigo-400',
  },
  judge: {
    card: 'border-violet-500/20 bg-violet-500/15',
    dot: 'border-violet-500/20 bg-violet-500/15 text-violet-400',
    badge: 'border-violet-500/20 bg-violet-500/15 text-violet-400',
  },
  report: {
    card: 'border-emerald-500/20 bg-emerald-500/15',
    dot: 'border-emerald-500/20 bg-emerald-500/15 text-emerald-400',
...
function stageStepTone(stepKey: string, state: StepStatus) {
  if (state === 'failed') {
    return {
      card: 'border-red-500/20 bg-red-500/15',
      dot: 'border-red-500/20 bg-red-500/15 text-red-400',
      badge: 'border-red-500/20 bg-red-500/15 text-red-400',
    };
  }
  if (state === 'running') {
    return {
      card: 'border-blue-500/20 bg-blue-500/15',
      dot: 'border-blue-500 bg-blue-500 text-white',
      badge: 'border-blue-500/20 bg-blue-500/15 text-blue-400',
    };
  }
  if (state === 'completed') {
    return STAGE_STEP_TONES[stepKey] || {
      card: 'border-theme-border bg-theme-elevated',
      dot: 'border-theme-border bg-theme-elevated text-theme-text-secondary',
      badge: 'border-theme-border bg-theme-elevated text-theme-text-secondary',
    };
  }
  return {
    card: 'border-theme-border bg-theme-elevated',
    dot: 'border-theme-border bg-theme-elevated text-theme-text-muted',
    badge: 'border-theme-border bg-theme-elevated text-theme-text-muted',
  };
}
...
export const DataflowAnalysisTaskDetailPage: React.FC<{ projectId: string; taskId: string; onBack: () => void }> = ({ projectId, taskId, onBack }) => {
 ...
 <button onClick={handleBack} className="mt-1 rounded-2xl border border-theme-border bg-theme-surface p-3 text-theme-text-secondary hover:bg-theme-elevated"><ArrowLeft size={18} /></button>
          <div className="min-w-0">
            ...
            <p className="mt-2 text-sm text-theme-text-muted">查看数据流追踪、智能体会话、结果产物和观测指标。</p>
          </div>
        </div>
...
className="form-select ml-2 text-xs"
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <div className="flex items-center gap-2 rounded-xl border border-theme-border bg-theme-surface px-2 py-1.5 text-xs font-semibold text-theme-text-secondary">
                    <button
                      ...
                      className="rounded-lg border border-theme-border px-2.5 py-1 text-xs font-semibold text-theme-text-secondary transition hover:bg-theme-elevated disabled:cursor-not-allowed disabled:opacity-40"
                    >
                      上一页
                    </button>
                    <span className="min-w-16 text-center text-xs font-bold text-theme-text-muted">
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-4">
                        <div>
                          ...
                          <div className="mt-2 flex flex-wrap gap-2 text-xs">
                            ...
                            <span className="rounded-full border border-theme-border bg-theme-elevated px-3 py-1 font-bold text-theme-text-secondary">{stageLabel(selectedEvaluationRound.stage)}</span>
                            <span className="rounded-full border border-theme-border bg-theme-elevated px-3 py-1 font-mono font-bold text-theme-text-secondary">阶段轮次 {selectedEvaluationRound.stage_round ?? '-'}</span>
                          </div>
                        </div>
 ...
 <section className="rounded-2xl border border-theme-border bg-theme-surface p-5">
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <span className="rounded-full border border-theme-border bg-theme-elevated px-3 py-1 text-xs font-bold text-theme-text-secondary">{selectedEvaluationRound.judges?.length || 0} 个 Judge</span>
                        </div>
                        <div className="mt-4 space-y-3">
                            ...
                            <div key={`${judge.judge_id || index}-${judge.model || ''}`} className="rounded-2xl border border-theme-border bg-theme-surface p-4">
                              <div className="flex flex-wrap items-center justify-between gap-2">
                                ...
                                <div className="flex flex-wrap gap-2 text-[11px]">
                                  <span className={`rounded-full px-2 py-0.5 font-bold ${judge.passed ? 'bg-emerald-500/15 text-emerald-400' : 'bg-rose-500/15 text-rose-400'}`}>{judge.passed ? '通过' : '未通过'}</span>
 ...
 <section className="rounded-2xl border border-theme-border bg-theme-surface p-5">
                    ...
                    <div className="mt-4 overflow-auto rounded-2xl border border-theme-border">
                      <table className="min-w-full divide-y divide-theme-border text-left text-xs">
                        ...
                        <tbody className="divide-y divide-theme-border bg-theme-elevated">
                          {filteredRounds.map((round) => (
                            <tr key={evaluationRoundKey(round)} onClick={() => setSelectedEvaluationRoundKey(evaluationRoundKey(round))} className="cursor-pointer hover:bg-theme-elevated">
                              ...
                              <td className="px-3 py-3 font-mono text-theme-text-secondary">{round.module_name || round.function || round.func || round.entry || '-'}</td>
                      ...
                      {filteredRounds.length === 0 ? <div className="px-4 py-10 text-center text-sm text-theme-text-muted">没有符合过滤条件的轮次</div> : null}
                    </div>
                  </section>
                )}
              </>
