      ...
      {!loading ? (
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-[360px_minmax(0,1fr)]">
          ...
          <section className="rounded-lg p-4 space-y-3" style={{ backgroundColor: LK.surface, border: `1px solid ${LK.border}` }}>
            <label className="block text-sm" style={{ color: LK.body }}>名称<input className="form-input mt-1 w-full" value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} /></label>
            <label className="block text-sm" style={{ color: LK.body }}>分类<input className="form-input mt-1 w-full" value={form.category} onChange={(e) => setForm((p) => ({ ...p, category: e.target.value }))} /></label>
            <label className="block text-sm" style={{ color: LK.body }}>描述<input className="form-input mt-1 w-full" value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} /></label>
            <label className="block text-sm" style={{ color: LK.body }}>变量（逗号分隔）<input className="form-input mt-1 w-full" value={form.variables} onChange={(e) => setForm((p) => ({ ...p, variables: e.target.value }))} /></label>
