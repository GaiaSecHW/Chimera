...
export const VulnVerifyConfigPage: React.FC<{ projectId: string; embedded?: boolean }> = ({ projectId, embedded = false }) => {
  ...
  return (
    <div className={containerClass}>
      <section className="rounded-xl border border-slate-200 bg-theme-surface p-6 shadow-sm">
        <PageHeader
          ...
          actions={
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-theme-surface px-3 py-2 text-sm font-bold text-theme-text-primary hover:bg-theme-elevated disabled:cursor-not-allowed disabled:opacity-60"
            >
        ...
        <div className="mt-6 grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
          <div className="rounded-2xl border border-slate-200 bg-theme-elevated p-5">
            <label className="block">
              <div className="text-sm font-semibold text-theme-text-primary">默认模型 default_model</div>
              <select
                value={defaultModel}
          ...
          <aside className="rounded-2xl border border-slate-200 bg-theme-surface p-5 text-sm text-theme-text-secondary">
            <div className="text-xs font-semibold uppercase tracking-[0.18em] text-theme-text-muted">Effective</div>
            <div className="mt-3 text-xs text-theme-text-muted">当前有效默认模型</div>
            <div className="mt-1 break-all font-mono text-sm font-semibold text-theme-text-primary">
              {effectiveDefaultModel || defaultModel || '继承 ConfigCenter 全局默认'}
            </div>
            <div className="mt-4 text-xs text-theme-text-muted">来源</div>
            <div className="mt-1 font-bold text-theme-text-primary">{sourceLabel(source)}</div>
            <div className="mt-4 text-xs text-theme-text-muted">最近更新</div>
            <div className="mt-1 text-theme-text-primary">{updatedBy || '-'}</div>
            <div className="mt-1 font-mono text-xs text-theme-text-muted">{formatDate(updatedAt)}</div>
          </aside>
        </div>
