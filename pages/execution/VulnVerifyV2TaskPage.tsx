...
import { Eye, Loader2, RefreshCw, RotateCcw, Search, X } from 'lucide-react';
...
export const VulnVerifyV2TaskPage: React.FC<{ projectId: string }> = ({ projectId }) => {
  ...
  const unresolvedVulnsFromStats = Number(stats?.unresolved ?? 0);
              ...
              {tasks.map((task) => {
                ...
                <tr key={task.id} className={executionTableInteractiveRowClassName} onClick={() => void openDetail(task.id)}>
                  <ExecutionTableTd>
                    ...
                    <div className="mt-1 font-mono text-[11px] text-theme-text-muted">{task.vuln_id || task.case_id || '-'}</div>
                  </ExecutionTableTd>
