...
export const atomicCapabilityCatalog: AtomicCapabilityDescriptor[] = [
  ...
  },
  {
    id: 'vuln-verify-v2',
    name: '漏洞验证v2',
    summary: '面向漏洞中心 case 的 v2 验证引擎，支持 code_root + raw_report 创建任务、四维判定、raw_result 完整归档。',
    inputDescription: 'case_id、代码根目录 code_root、漏洞报告 markdown raw_report、模型覆盖（可选）。',
    outputDescription: 'confirmed/ruled_out/unresolved 结论、四维 dimensions、ruled_out_by、root_cause_summary、pi stdout/stderr。',
    tags: ['漏洞验证', 'v2', 'raw_report', '四维判定'],
    viewId: 'pentest-vuln-verify-v2',
    icon: ShieldCheck,
    serviceName: 'secflow-app-vuln-verify-v2',
    k8sServiceHost: 'secflow-app-vuln-verify-v2',
    port: 80,
    apiPrefix: '/api/app/vuln-verify-v2',
    docsPath: '/docs',
    openapiPath: '/openapi.json',
    redocPath: '/redoc',
    platformDocsCandidates: ['/api/app/vuln-verify-v2/docs'],
    platformOpenapiCandidates: ['/api/app/vuln-verify-v2/openapi.json'],
    platformRedocCandidates: ['/api/app/vuln-verify-v2/redoc'],
    apiGroups: [
      {
        groupName: '健康与任务',
        description: '服务健康、v2 任务创建、列表与详情查询。',
        endpoints: [
          { method: 'GET', path: '/health', purpose: '获取服务健康状态。', requestSummary: '无请求体。', responseSummary: '返回 status、service、version。' },
          { method: 'POST', path: '/projects/{project_id}/tasks', purpose: '创建 v2 漏洞验证任务。', requestSummary: '提交 case_id、code_root、raw_report、model（可选）。', responseSummary: '返回任务状态、work_dir 与执行配置。' },
          { method: 'GET', path: '/projects/{project_id}/tasks', purpose: '分页查询任务。', requestSummary: '支持 status、search、limit、offset。', responseSummary: '返回任务列表和总数。' },
          { method: 'GET', path: '/projects/{project_id}/tasks/{task_id}', purpose: '查看任务详情。', requestSummary: '路径参数 project_id/task_id。', responseSummary: '返回任务、attempts、results。' },
        ],
      },
      {
        groupName: '结果与控制',
        description: '查看 v2 结果、artifact 和控制任务生命周期。',
        endpoints: [
          { method: 'GET', path: '/projects/{project_id}/tasks/{task_id}/results', purpose: '获取 verifier 结果。', requestSummary: '路径参数 project_id/task_id。', responseSummary: '返回 verdict、dimensions、raw_result。' },
          { method: 'GET', path: '/projects/{project_id}/tasks/{task_id}/artifacts', purpose: '列出 result.json/stdout/stderr。', requestSummary: '路径参数 project_id/task_id。', responseSummary: '返回 artifact 列表。' },
          { method: 'POST', path: '/projects/{project_id}/tasks/{task_id}/terminate', purpose: '终止任务。', requestSummary: '路径参数 project_id/task_id。', responseSummary: '返回操作结果。' },
          { method: 'POST', path: '/projects/{project_id}/tasks/{task_id}/rerun', purpose: '重跑终态任务。', requestSummary: '路径参数 project_id/task_id。', responseSummary: '返回操作结果。' },
        ],
      },
    ],
  },
