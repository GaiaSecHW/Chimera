...
export const TaskOriginInline: React.FC<{ origin: OriginInfo; compact?: boolean }> = ({ origin, compact = false }) => {
  ...
  if (!isBinarySecurity) {
    return (
      <span className={`inline-flex items-center rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 font-bold text-slate-600 ${compact ? 'text-[10px]' : 'text-xs'}`}>
        手动任务
      </span>
    );
  }
