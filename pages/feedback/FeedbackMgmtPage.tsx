...
export function FeedbackMgmtPage() {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold text-theme-text-primary mb-4">意见管理</h1>
      <div className="bg-theme-surface border border-theme-border rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-theme-border text-theme-text-muted bg-theme-elevated">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={6} className="text-center py-8 text-theme-text-muted">加载中...</td></tr>
            ) : feedbacks.length === 0 ? (
              <tr><td colSpan={6} className="text-center py-8 text-theme-text-muted">暂无意见反馈</td></tr>
            ) : (
              feedbacks.map((fb) => (
                <tr key={fb.id} className="border-b border-theme-border hover:bg-theme-elevated">
                  ...
                  <td className="py-3 px-4 text-center">
                    <button
                      ...
                      className="px-3 py-1 rounded text-blue-600 border border-blue-600/30 hover:bg-blue-600/10"
                    >
                      处理
                    </button>
                  </td>
