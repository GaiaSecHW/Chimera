...
const toMetadataObject = (entries: MetadataEntry[]) =>
  entries.reduce<Record<string, any>>((acc, entry) => {
...
export const AiwfTriggersPage: React.FC<{
  ...
  const loadTriggers = async () => {
    try {
      setLoading(true);
      const items = await orchestrationApi.aiAgentFramework.listTriggerTasks();
  ...
  const ensureDirectory = async (subId: number, parentId: number | null, name: string): Promise<number> => {
    const payload = parentId === null
      ? await assetApi.fileserver.getSubprojectChildren(projectId, subId)
      : await assetApi.fileserver.getDirectoryChildren(projectId, parentId);
    const existed = payload.directories.find((item) => item.name === name);
    if (existed) return existed.id;
    const created = await assetApi.fileserver.createDirectory({
      project_id: projectId,
      subproject_id: subId,
      parent_id: parentId,
      name,
  ...
  const handleUploadFiles = async (files: FileList | null, isFolder: boolean) => {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        const result = await assetApi.fileserver.uploadFile(
          {
            project_id: projectId,
            subproject_id: subId,
  ...
  const handleCreateTrigger = async () => {
    ...
    try {
      await orchestrationApi.aiAgentFramework.createTriggerTask(selectedDefinitionIdState, {
        input_tasks,
        priority: priority.trim() ? Number(priority) : undefined,
      });
  ...
  const handleCancelTrigger = async (triggerId: string) => {
    try {
      await orchestrationApi.aiAgentFramework.cancelTriggerTask(triggerId);
  ...
  const handleRetryTrigger = async (triggerId: string) => {
    try {
      await orchestrationApi.aiAgentFramework.retryTriggerTask(triggerId);
