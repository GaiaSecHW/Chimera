...
const IMPORT_STATUS_STYLES: Record<string, string> = {
  ...
  skipped: 'bg-amber-500/15 text-amber-400 border-amber-500/20',
...
export const DepartmentMemberPage: React.FC = () => {
  ...
  const fetchUserPermissions = async () => {
    try {
      const data = await platformApi.org.getUserPermissions();
  ...
  const getRoleBadgeStyle = (role: string): string => {
    const styles: Record<string, string> = {
      leader: 'bg-amber-500/15 text-amber-400 border-amber-500/20',
  ...
  const fetchDepartments = async () => {
    try {
      const data = await platformApi.org.listDepartments();
      setDepartments(data || []);
  ...
  const fetchMembers = async (departmentId: number) => {
    setLoading(true);
    try {
      const data = await platformApi.org.getDepartmentMembers(departmentId);
      setMembers(data || []);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleEditMember = async (e: React.FormEvent) => {
    ...
    setFormLoading(true);
    try {
      await platformApi.org.updateDepartmentMember(selectedMember.id, { role: formData.role });
  ...
  const handleRemoveMember = async (memberId: number) => {
    ...
    if (!confirmed) return;
    try {
      await platformApi.org.removeDepartmentMember(memberId);
      if (selectedDepartmentId) {
        await fetchMembers(selectedDepartmentId);
      }
    } catch (err: any) {
  ...
  const handleMoveMember = async (e: React.FormEvent) => {
    ...
    setFormLoading(true);
    try {
      await platformApi.org.updateDepartmentMember(selectedMember.id, { department_id: parseInt(moveDepartmentId, 10) });
  ...
  const handleDownloadImportTemplate = async () => {
    try {
      const blob = await platformApi.org.downloadDepartmentMemberImportTemplate();
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = 'chimera-department-member-import-template.csv';
      link.click();
  ...
  const handlePreviewImport = async () => {
    ...
    try {
      const preview = await platformApi.org.previewDepartmentMemberImport({
  ...
  const handleCommitImport = async () => {
    ...
    setImportLoading(true);
    try {
      const result = await platformApi.org.commitDepartmentMemberImport({
        department_id: selectedDepartmentId,
        file_content_base64: importFileContentBase64,
        filename: importFileName,
        mode: importMode,
 ...
 className="w-full px-8 pr-14 py-5 bg-theme-elevated backdrop-blur border border-theme-border rounded-xl text-sm text-left outline-none focus:ring-4 ring-blue-500/5 transition-all font-medium text-theme-text-secondary"
            ...
            <span className="pointer-events-none absolute right-6 top-1/2 -translate-y-1/2 text-theme-text-muted">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-8">
                  <div className="rounded-xl border border-theme-border bg-theme-elevated p-6 space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      {!isAdmin() && <p className="text-xs text-theme-text-muted font-medium">普通管理员仅支持“已存在则跳过”。</p>}
                    </div>
