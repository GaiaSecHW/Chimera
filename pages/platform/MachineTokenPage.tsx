...
export const MachineTokenPage: React.FC = () => {
  ...
  return (
    <div className="h-full overflow-y-auto bg-theme-app px-6 py-8 md:px-8 xl:px-10">
      <div className="flex w-full flex-col gap-6 pb-24">
        <section className="relative overflow-hidden rounded-xl border border-slate-200/80 bg-[linear-gradient(135deg,_#0f172a,_#1e293b_55%,_#1d4ed8)] px-8 py-8 text-white md:px-10">
 ...
 <div className="flex h-16 w-16 items-center justify-center rounded-lg bg-theme-elevated text-sky-100 shadow-inner shadow-white/5">
                  <Cpu size={30} />
                </div>
                <div className="space-y-2">
                  <h2 className="text-3xl font-bold tracking-tight md:text-4xl">机机 Token 管理</h2>
 ...
 className="inline-flex items-center gap-2 rounded-2xl border border-theme-border bg-theme-elevated px-4 py-3 text-sm font-medium text-white transition hover:bg-slate-50/15"
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-2xl bg-theme-surface px-5 py-3 text-sm font-semibold text-theme-text-primary transition hover:bg-theme-elevated"
              >
                <Plus size={16} />
                申请新 Token
              </button>
 ...
 <div className="rounded-xl bg-blue-600 px-6 py-6 text-white">
            <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-blue-100/80">已签发凭证</p>
            <p className="mt-4 text-5xl font-bold">{tokens.length}</p>
 ...
 <div className="rounded-xl border border-theme-border bg-theme-surface px-6 py-6">
            <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-theme-text-muted">活跃凭证</p>
            <p className="mt-4 text-4xl font-bold text-theme-text-primary">{activeTokens}</p>
            <p className="mt-4 text-sm font-medium text-theme-text-muted">当前处于启用状态、允许服务调用的凭证。</p>
          </div>
 <div className="rounded-xl border border-theme-border bg-theme-surface px-6 py-6">
            <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-theme-text-muted">永久凭证</p>
 ...
 <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-theme-surface text-sky-400">
              ...
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-sky-500">使用建议</p>
                <p className="mt-1 text-lg font-semibold text-sky-300">每个节点独立申请</p>
              </div>
            </div>
 ...
 <section className="rounded-xl border border-slate-200/80 bg-theme-surface p-5 backdrop-blur">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-3">
              <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-blue-500/15 text-blue-400">
                <Search size={20} />
              </div>
              <div>
                <h3 className="text-lg font-semibold text-theme-text-primary">检索凭证</h3>
 ...
 <section className="overflow-hidden rounded-xl border border-slate-200/80 bg-theme-surface">
          <div className="border-b border-theme-border px-6 py-5 md:px-8">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-theme-elevated text-theme-text-secondary">
                  <Key size={22} />
                </div>
                <div>
                  <h3 className="text-lg font-semibold text-theme-text-primary">Token 列表</h3>
            ...
            const columns: DataTableColumn<MachineToken>[] = [
              {
                ...
                render: (token) => (
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-theme-text-primary">{token.machine_code}</p>
              ...
              },
              {
                ...
                header: '用途描述',
                render: (token) => (
                  <div className="max-w-[280px] rounded-lg border border-theme-border bg-theme-surface px-4 py-3 text-sm font-medium leading-6 text-theme-text-muted">
              ...
              },
              {
                ...
                header: '过期策略',
                render: (token) => (
                  <div className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-medium ${token.expires_at ? 'bg-amber-500/15 text-amber-400' : 'bg-emerald-500/15 text-emerald-400'}`}>
                    {token.expires_at ? (
                      <>
              ...
              },
              {
                ...
                render: (token) => (
                  <button
                    ...
                    className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.18em] transition ${
                      token.is_active
                ...
                render: (token) => (
                  <div className="flex justify-end gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-2xl border border-theme-border bg-theme-surface px-4 py-3 text-sm font-medium text-theme-text-secondary transition hover:border-blue-500/20 hover:text-blue-400"
                      title="重新生成凭证值"
                    >
                      <Zap size={14} />
                      重签发
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-2xl bg-rose-500/15 px-4 py-3 text-sm font-semibold text-rose-400 transition hover:bg-rose-600 hover:text-white"
                      title="彻底删除"
                    >
                      <Trash2 size={14} />
                      撤销
              ...
              <DataTable<MachineToken>
                ...
                empty={
                  <div className="px-8 py-32 text-center">
                    ...
                    <p className="mt-5 text-base font-semibold text-theme-text-muted">暂无匹配的机机凭证</p>
        ...
        <Modal open={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} className="max-w-2xl">
              <div className="flex items-center justify-between border-b border-theme-border px-8 py-7">
                <div className="flex items-center gap-4">
 <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-theme-surface text-white">
                    {lastCreatedToken ? <ShieldCheck size={22} /> : <Plus size={22} />}
                  </div>
                  <div>
                    <h3 className="text-xl font-semibold text-theme-text-primary">{lastCreatedToken ? '新凭据已生成' : '申请机器 Token'}</h3>
                  </div>
                </div>
              ...
              {lastCreatedToken ? (
                <div className="space-y-6 overflow-y-auto px-8 py-8">
                  <div className="flex items-start gap-4 rounded-xl border border-emerald-500/20 bg-emerald-500/15 p-5">
                    <ShieldCheck className="mt-1 shrink-0 text-emerald-400" size={20} />
                    <div>
                      <h4 className="text-sm font-semibold text-emerald-400">凭证托管就绪</h4>
                      <p className="mt-1 text-sm font-medium leading-6 text-emerald-700/80">请立即保存下方原始 Token 值，此窗口关闭后将无法再次通过界面获取。</p>
                    </div>
                  </div>
                  <div className="relative">
                    <div className="break-all rounded-xl bg-theme-surface p-6 font-mono text-sm leading-7 text-sky-300 shadow-inner">
                      {lastCreatedToken}
                    </div>
                    <button
 ...
 className={`absolute right-4 top-4 rounded-xl p-3 transition ${copied ? 'bg-emerald-500 text-white' : 'bg-theme-elevated text-white hover:bg-theme-elevated'}`}
                  ...
                  <button
                    ...
                    className="w-full rounded-2xl bg-theme-surface py-4 text-sm font-medium text-white transition hover:bg-theme-elevated"
                  >
                    我已安全保存凭据
                  </button>
                </div>
                ...
                <form onSubmit={handleCreate} className="space-y-6 overflow-y-auto px-8 py-8">
                  ...
                  <button
                    ...
                    className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-theme-surface py-4 text-sm font-medium text-white transition hover:bg-theme-elevated disabled:cursor-not-allowed disabled:opacity-50"
                  >
