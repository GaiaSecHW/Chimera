...
export const UserMgmtPage: React.FC = () => {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const data = await platformApi.auth.listUsers();
      setUsers(data || []);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    setFormLoading(true);
    try {
      await platformApi.auth.changePasswordAdmin(selectedUser.id, resetData);
  ...
  const toggleStatus = async (user: UserInfo) => {
    try {
      await platformApi.auth.updateUser(user.id, { is_active: !user.is_active });
      await fetchUsers();
    } catch (err: any) {
      alert(err.message);
    }
  ...
  const handleDeleteUser = async (user: UserInfo) => {
    ...
    if (!confirmed) return;
    await platformApi.auth.deleteUser(user.id);
    await fetchUsers();
  };

  const handleDownloadTemplate = async () => {
    try {
      const blob = await platformApi.auth.downloadUserImportTemplate();
  ...
  const handlePreviewImport = async () => {
    ...
    try {
      const preview = await platformApi.auth.previewUserImport({
  ...
  const handleCommitImport = async () => {
    ...
    setImportLoading(true);
    try {
      const result = await platformApi.auth.commitUserImport({
        csv_content: importCsvContent || undefined,
        file_content_base64: importFileContentBase64 || undefined,
        filename: importFileName,
        default_password: importDefaultPassword || undefined,
 ...
 <div className="p-3 bg-gradient-to-br from-blue-600 via-cyan-500 to-sky-500 text-white rounded-2xl">
            ...
            <div>
              <h2 className="text-3xl font-black text-theme-text-primary tracking-tight">用户账号管理</h2>
            </div>
          </div>
        </div>
        <div className="flex gap-4">
 <button onClick={() => void fetchUsers()} className="p-4 bg-theme-bg-app backdrop-blur border border-theme-border text-theme-text-muted rounded-2xl hover:bg-theme-bg-app transition-all active:scale-95">
 ...
 <div className="bg-theme-bg-app backdrop-blur p-8 rounded-[3rem] border border-emerald-500/20 flex flex-col justify-between">
          <p className="text-theme-text-muted text-[10px] font-black uppercase tracking-widest">活跃账号</p>
          <h3 className="text-4xl font-black mt-4 text-green-400">{userStats.active}</h3>
 ...
 <div className="bg-theme-bg-app backdrop-blur p-8 rounded-[3rem] border border-indigo-500/20 flex flex-col justify-between">
          <p className="text-theme-text-muted text-[10px] font-black uppercase tracking-widest">账号分布</p>
          <div className="mt-4 grid grid-cols-3 gap-4">
            <div className="rounded-[1.75rem] bg-indigo-500/15 px-5 py-4">
              ...
              <p className="mt-2 text-3xl font-black text-indigo-400">{userStats.ordinaryAdmin}</p>
            </div>
            <div className="rounded-[1.75rem] bg-fuchsia-500/15 px-5 py-4">
              <p className="text-[10px] font-black uppercase tracking-widest text-fuchsia-400">开发者</p>
              <p className="mt-2 text-3xl font-black text-fuchsia-400">{userStats.developer}</p>
            </div>
            <div className="rounded-[1.75rem] bg-blue-500/15 px-5 py-4">
              ...
              <p className="mt-2 text-3xl font-black text-blue-400">{userStats.departmentBound}</p>
            </div>
          </div>
        </div>
 <div className="bg-theme-bg-app backdrop-blur p-8 rounded-[3rem] border border-theme-border flex items-center gap-8">
          ...
          <div>
            <h4 className="text-lg font-black text-theme-text-primary">导入与身份治理</h4>
            <p className="text-sm text-theme-text-muted mt-1 font-medium">支持单个创建和 Excel/CSV 批量导入。批量导入会先预校验用户名、角色和部门归属，再执行落库。</p>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {loading ? (
                ...
                <tr><td colSpan={5} className="py-32 text-center text-theme-text-muted font-bold">暂无匹配的用户数据</td></tr>
              ) : paginatedUsers.map((user) => (
                <tr key={user.id} className="hover:bg-theme-elevated transition-all group">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-blue-500/15 text-blue-400 rounded-2xl flex items-center justify-center font-black shadow-inner">
                        {user.username[0].toUpperCase()}
                      </div>
                      <div>
                        <p className="text-sm font-black text-theme-text-primary">{user.username}</p>
                        <p className="text-[10px] text-theme-text-muted font-mono mt-0.5">UID: {user.id.toString().padStart(5, '0')}</p>
                      </div>
                    </div>
                  </td>
                    ...
                    <div className="space-y-3">
                      <div className="flex flex-wrap gap-1">
                        ...
                        {user.department_name && (
                          <span className="text-[10px] font-black bg-cyan-500/15 text-cyan-400 px-2.5 py-1 rounded-lg border border-cyan-500/20">
                            {user.department_name}
                          </span>
                        )}
                      </div>
                      ...
                      {user.role?.length > 0 ? user.role.map((r) => (
                        <span key={r} className="text-[10px] font-black bg-blue-500/15 text-blue-400 px-2.5 py-1 rounded-lg border border-blue-500/20 uppercase">{r}</span>
                      )) : <span className="text-[10px] font-bold text-theme-text-faint italic">None Assigned</span>}
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-6 text-center">
                    <button onClick={() => void toggleStatus(user)} className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase border transition-all ${user.is_active ? 'bg-green-500/15 text-green-400 border-green-500/20 hover:bg-red-500/15 hover:text-red-400 hover:border-red-500/20' : 'bg-red-500/15 text-red-400 border-red-500/20 hover:bg-green-500/15 hover:text-green-400 hover:border-green-500/20'}`}>
 ...
 className="p-3 bg-red-500/15 text-red-400 border border-transparent hover:border-red-500/20 rounded-xl transition-all"
          ...
          {!loading && filteredUsers.length > 0 && (
            <div className="flex flex-col gap-4 border-t border-theme-border px-8 py-5 md:flex-row md:items-center md:justify-between">
 ...
 <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center text-white">
                ...
                <div>
                  <h3 className="text-xl font-black text-theme-text-primary">创建新用户</h3>
                  <p className="text-[10px] text-theme-text-muted font-bold uppercase mt-0.5">New users default to ordinary user</p>
                </div>
              </div>
            ...
            <form onSubmit={handleCreate} className="p-10 space-y-6">
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-theme-text-muted uppercase tracking-widest ml-1">用户名 *</label>
                <input
                  required
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-theme-text-muted uppercase tracking-widest ml-1">初始密码 *</label>
                <input
                  type="password"
                  required
                  placeholder="••••••••"
 ...
 <button disabled={formLoading} className="w-full py-5 bg-blue-600 text-white rounded-2xl font-black hover:bg-blue-700 transition-all flex items-center justify-center gap-3">
      ...
      {selectedUser && <Modal open={isResetModalOpen} onClose={() => setIsResetModalOpen(false)} className="max-w-md">
            ...
            <form onSubmit={handleResetPassword} className="p-10 space-y-6">
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-theme-text-muted uppercase tracking-widest ml-1">管理员密码验证 *</label>
                <input
                  type="password"
                  required
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-theme-text-muted uppercase tracking-widest ml-1">新密码 *</label>
                <input
                  type="password"
                  required
 ...
 <div className="w-14 h-14 rounded-3xl bg-blue-600 text-white flex items-center justify-center">
                ...
                <div>
                  <h3 className="text-2xl font-black text-theme-text-primary">批量导入用户</h3>
                </div>
              </div>
              <button onClick={closeImportModal} className="p-3 text-theme-text-faint hover:text-theme-text-secondary"><X size={28} /></button>
            </div>
            ...
            <div className="px-10 pt-6 flex gap-3 flex-wrap">
              {['upload', 'preview', 'result'].map((stage, index) => {
                ...
                return (
                  <div key={stage} className={`px-4 py-2 rounded-full border text-[11px] font-black uppercase tracking-[0.2em] ${active ? 'bg-blue-600 text-white border-blue-600' : passed ? 'bg-emerald-500/15 text-emerald-400 border-emerald-500/20' : 'bg-theme-bg-app text-theme-text-muted border-theme-border'}`}>
                    {stage}
                  </div>
                );
              })}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-8">
                  <div className="rounded-[2rem] border border-theme-border bg-slate-50/70 p-8 space-y-6">
                    <div className="space-y-2">
                      <h4 className="text-xl font-black text-theme-text-primary">1. 准备导入文件</h4>
                      <p className="text-sm text-theme-text-muted font-medium">下载模板后直接按示例填写即可，支持上传`.xlsx` 或`.csv` 文件，系统会先预校验再导入。</p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest text-theme-text-muted">统一初始密码</label>
                        <input
                          type="password"
                          placeholder="可选，不填则按行密码或随机密码"
                          className="w-full px-5 py-4 rounded-2xl bg-theme-bg-app border border-theme-border outline-none focus:ring-4 ring-blue-500/10 font-semibold text-theme-text-primary"
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest text-theme-text-muted">首次登录策略</label>
                        <button
                          type="button"
                          onClick={() => setImportForcePasswordChange((value) => !value)}
                          className={`w-full rounded-2xl border px-5 py-4 text-left transition-all ${importForcePasswordChange ? 'border-amber-500/20 bg-amber-500/15 text-amber-400' : 'border-theme-border bg-theme-bg-app text-theme-text-secondary'}`}
                        >
 ...
 <button onClick={() => void handleDownloadTemplate()} className="px-6 py-4 rounded-2xl bg-theme-bg-app border border-theme-border font-black text-theme-text-secondary flex items-center gap-3 hover:bg-theme-elevated">
                        <Download size={18} />
                        下载模板
                      </button>
 <button onClick={() => fileInputRef.current?.click()} className="px-6 py-4 rounded-2xl bg-blue-600 text-white font-black flex items-center gap-3 hover:bg-blue-700">
                    ...
                    <div className="rounded-[2rem] border border-dashed border-theme-border bg-theme-bg-app p-6">
                      ...
                      <p className="mt-2 text-sm text-theme-text-muted">模板已内置示例和填写说明，支持字段：`username,password,platform_role,role_names,department_name,department_role,is_active`。如果行内不填密码，可用上面的“统一初始密码”或随机密码。</p>
                    </div>
                    <button disabled={importLoading || !importFileName} onClick={() => void handlePreviewImport()} className="w-full py-5 rounded-2xl bg-theme-surface text-white font-black flex items-center justify-center gap-3 disabled:opacity-50">
 ...
 <div className="rounded-[2rem] border border-theme-border bg-theme-bg-app p-8 space-y-5">
                    <h4 className="text-xl font-black text-theme-text-primary">导入规则</h4>
              ...
              {importStage === 'result' && importResult && (
                <div className="space-y-6">
                  ...
                  {generatedPasswords.length > 0 && (
                    <div className="rounded-[2rem] border border-amber-500/20 bg-amber-500/15 p-6">
                      <h4 className="text-lg font-black text-amber-300">自动生成的初始密码</h4>
                      <p className="mt-2 text-sm font-medium text-amber-400">这些密码只会在本次导入结果中展示一次，请尽快通知相关用户首次登录后修改。</p>
                      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                        {generatedPasswords.map((row) => (
                          <div key={row.row_no} className="rounded-2xl bg-theme-bg-app border border-amber-500/20 px-4 py-3">
                            <p className="text-xs uppercase tracking-[0.2em] font-black text-amber-500">Row {row.row_no}</p>
                            <p className="mt-1 text-sm font-black text-theme-text-primary">{row.username}</p>
                            <p className="mt-1 font-mono text-sm text-theme-text-secondary">{row.generated_password}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="flex justify-between items-center gap-4">
                    <div>
                      ...
                      <p className="text-sm text-theme-text-muted font-medium">成功与失败都会逐行记录，失败行可修复后再次导入。</p>
                    </div>
        ...
        {rows.map((row) => (
          <tr key={row.row_no} className="align-top">
            ...
            <td className="px-6 py-5">
              ...
              {row.generated_password && <p className="mt-2 text-xs font-mono text-amber-400">自动密码: {row.generated_password}</p>}
            </td>
            <td className="px-6 py-5 text-sm text-theme-text-secondary font-medium">
