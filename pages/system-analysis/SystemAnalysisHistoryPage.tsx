...
export const SystemAnalysisHistoryPage: React.FC<{ projectId: string }> = ({ projectId }) => {
  ...
  const loadTasks = async () => {
    ...
    setLoading(true);
    try {
      const resp = await api.systemAnalysis.listTasks({ project_id: projectId, page: 1, per_page: 50 });
  ...
  const loadDetail = async (taskId: string) => {
    ...
    try {
      const [d, n, r] = await Promise.all([
        api.systemAnalysis.getTask(taskId),
        api.systemAnalysis.getTaskNodes(taskId),
        api.systemAnalysis.getTaskReport(taskId).catch(() => null),
      ]);
      setDetail(d);
      setNodes(n.items || []);
      setReport(r);
              ...
              <>
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  ...
                  <div className="flex gap-2">
                    <button className="rounded-lg border border-slate-200 px-3 py-2 text-sm" onClick={() => selectedTaskId && void api.systemAnalysis.rerunTask(selectedTaskId).then(() => { notify('已提交重跑任务', 'success'); void loadTasks(); })}>重跑</button>
                    <button className="rounded-lg border border-red-200 px-3 py-2 text-sm text-red-600" onClick={() => selectedTaskId && void api.systemAnalysis.cancelTask(selectedTaskId).then(() => { notify('任务已取消', 'success'); void loadTasks(); void loadDetail(selectedTaskId); })}>取消</button>
                  </div>
                </div>
